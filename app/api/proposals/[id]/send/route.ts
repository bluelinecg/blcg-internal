 ...
 * Generates the proposal PDF and sends it as an email attachment via Gmail.
 *
...
export async function POST(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const {
      ...
      body    = buildDefaultBody(proposal.proposalNumber, proposal.title),
      from    = 'ryan@bluelinecg.com',
    } = parsed.data;

    // Generate PDF buffer
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const pdfBuffer = await renderToBuffer(React.createElement(ProposalPDF, { proposal }) as any);
...
function buildDefaultBody(proposalNumber: string, title: string): string {
  return [
    `Please find attached Proposal ${proposalNumber}: ${title}.`,
    '',
    'Review the proposal at your convenience and let us know if you have any questions.',
    '',
